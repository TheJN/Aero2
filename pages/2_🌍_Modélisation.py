...
#######################################################################################################################
########################################## PROJET AÉRODYNAMIQUE HYPERSONIQUE ##########################################
#######################################################################################################################
...
st.set_page_config(
    page_title="Projet d'aéronautique hypersonique",
    page_icon="👋",
)


### Sidebar des paramètres variables (choix utilisateur) ###
...
st.title('Evolution des grandeurs physiques')
...
with st.container():
    ...
    with col1:
        ...
        st.line_chart(pd.DataFrame(Liste_acceleration,Liste_Temps,columns=["Accélération"]))
    ...
    with col2:
        ...
        st.line_chart(pd.DataFrame(Liste_position_z,Liste_Temps,columns=["Altitude"]))

    with col3:
        st.write('Masse en fonciton du temps')
        st.line_chart(pd.DataFrame(Liste_masse,Liste_Temps,columns=["Masse"]))
...
with st.container():
    ...
    with col1:
        st.write('Pinf en fonciton du temps')
        st.line_chart(pd.DataFrame(Pinf,Liste_Temps,columns=["Pinf"]))

    with col2:
        st.write('Liste_Mach en fonciton du temps')
        st.line_chart(pd.DataFrame(Liste_Mach,Liste_Temps,columns=["Liste_Mach"]))

    with col3:
        st.write('Liste_vitesse en fonciton du temps')
        st.line_chart(pd.DataFrame(Liste_vitesse,Liste_Temps,columns=["Liste_vitesse"]))
...
with st.container():
    ...
    with col1:
        st.write('Liste_poussee en fonciton du temps')
        st.line_chart(pd.DataFrame(Liste_poussee,Liste_Temps,columns=["Liste_poussee"]))

    with col2:
        st.write('Liste_trainee en fonciton du temps')
        st.line_chart(pd.DataFrame(Liste_trainee,Liste_Temps,columns=["Liste_trainee"]))

    with col3:
        st.write('Liste_Cx en fonciton du temps')
        st.line_chart(pd.DataFrame(Liste_Cx,Liste_Temps,columns=["Liste_Cx"]))
