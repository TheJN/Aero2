...
Point = st.selectbox(
    'Choississez les variables à afficher',
...
### Pente de la géométrie ###
def calcul_theta(x,y,alpha) :
    theta = [] 
    for i in range(1,len(x)-1) :
        theta.append(np.arctan((y[i+1]-y[i])/(x[i+1]-x[i])) - alpha*np.pi/180) 
    return(theta)
...
# # ax.set_ylabel('Accélération (m/s^2)')
# ax.set_title('Profil du lanceur')
...
### Valeurs au choc ###
# delta = calcul_theta(xx,zz,alpha)
...
# st.write(Liste_P0)
# st.write(Liste_rho0)
# st.write(Liste_gamma0)
# st.write(Liste_Ti0)
# st.write(Liste_Pi0)
# st.write(Liste_rhoi0)
# st.write(Liste_mu0)
# st.write(Liste_Regime0)
...
with st.container():
    ...
    with col1:
        ...
        st.write('Accélérarion en fonciton du temps')
        st.line_chart(pd.DataFrame(Liste_T0,xx,columns=["Liste_T0"]))


        # # Tracer le graphique
        # fig, ax = plt.subplots()
        # ax.plot(chart_data['Temps'], chart_data['Acceleration'])
        # ax.set_xlabel('Temps (s)')
        # ax.set_ylabel('Accélération (m/s^2)')
    ...
    with col2:
        st.write('Altitude en fonction du temps')
        st.line_chart(pd.DataFrame(Liste_P0,xx,columns=["Altitude"]))

    with col3:
        st.write('Masse en fonciton du temps')
        st.line_chart(pd.DataFrame(Liste_rho0,xx,columns=["Masse"]))
...
with st.container():
    ...
    with col1:
        st.write('Liste_gamma0 en fonciton du temps')
        st.line_chart(pd.DataFrame(Liste_gamma0,xx,columns=["Liste_gamma0"]))

    with col2:
        st.write('Liste_Ti0 en fonciton du temps')
        st.line_chart(pd.DataFrame(Liste_Ti0,xx,columns=["Liste_Ti0"]))

    with col3:
        st.write('Liste_Pi0 en fonciton du temps')
        st.line_chart(pd.DataFrame(Liste_Pi0,xx,columns=["Liste_Pi0"]))
...
with st.container():
    ...
    with col1:
        st.write('Liste_rhoi0 en fonciton du temps')
        st.line_chart(pd.DataFrame(Liste_rhoi0,xx,columns=["Liste_rhoi0"]))

    with col2:
        st.write('Liste_mu0 en fonciton du temps')
        st.line_chart(pd.DataFrame(Liste_mu0,xx,columns=["Liste_mu0"]))

    with col3:
        st.write('Liste_Regime0 en fonciton du temps')
        st.line_chart(pd.DataFrame(Liste_Regime0,xx,columns=["Liste_Regime0"]))
        
        
        
        
st.header("caractéristiques du point")
carac = [z,vz,vx ,az ,ax, Pinf, M ,theta, m ,L, S ,g ,Cx ,poids, trainee_z, trainee_x, poussee_z ,poussee_x ,temps] 
